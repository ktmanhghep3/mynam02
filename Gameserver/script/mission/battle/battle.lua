...
function Battle:SendBTOpenInfo()
	local szMsg = string.format("Đại chiến Tống Kim sắp bắt đầu, hiện đang tiến hành báo danh, người muốn tham chiến hãy nhanh chóng tìm Quan Mộ Binh (chiến trường) hoặc dùng Chiêu Thư Tống Kim, thời gian báo danh còn: %d phút.", self.TIMER_SIGNUP / (Env.GAME_FPS * 60), self.LEVEL_LIMIT[1])
	KDialog.NewsMsg(0, Env.NEWSMSG_NORMAL, szMsg);
	--if Config.Mission_Battle_CheckNumAccount == 1 then
		--Battle:resetHWfile();
	--end
	ReBuyBattle:DemSoTongKim();
end
