...
function tbMap:OnEnter(szParam)
	...
	if nUseTime >= Boss.PhuDoCoThanh_CaoThu.MAX_DAILY_TIME then
		me.NewWorld(Boss.PhuDoCoThanh_CaoThu:GetLeaveMapPos());
		me.SetFightState(0);
		return;
	end
	...
	-- 加入当前地图的列表
	Boss.PhuDoCoThanh_CaoThu:AddPlayer(me.nId, 1);
	
	-- 地图对玩家影响
	Boss.PhuDoCoThanh_CaoThu:OnMapEffect(me.nId, 1);
...
local pPlayer = me;
	-- 清除地图效果
	Boss.PhuDoCoThanh_CaoThu:OnMapLeave(me.nId, 1);
			
	-- 移出当前地图的列表
	Boss.PhuDoCoThanh_CaoThu:RemovePlayer(me.nId);
...
function tbSafeIn1:OnPlayer()
	...
	me.NewWorld(2110, 1622, 3167);--Chiến đấu
