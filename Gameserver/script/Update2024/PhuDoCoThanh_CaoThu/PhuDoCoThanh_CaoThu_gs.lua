Require("\\script\\Update2024\\PhuDoCoThanh_CaoThu\\PhuDoCoThanh_CaoThu_def.lua");

if (not MODULE_GAMESERVER) then
	return 0;
end

local tbPhuDoCoThanh_CaoThu = Boss.PhuDoCoThanh_CaoThu;

tbPhuDoCoThanh_CaoThu.nMapId = 2110;
tbPhuDoCoThanh_CaoThu.MapPos_Random =
{
{tbPhuDoCoThanh_CaoThu.nMapId , 1833,3443},
{tbPhuDoCoThanh_CaoThu.nMapId , 1766,3340},
{tbPhuDoCoThanh_CaoThu.nMapId , 1615,3225},
{tbPhuDoCoThanh_CaoThu.nMapId , 1873,3664},
{tbPhuDoCoThanh_CaoThu.nMapId , 1924,3202},
};

-- 计时器触发
function tbPhuDoCoThanh_CaoThu:OnTimer()
		...
		for nPlayerId, tbPlayerMap in pairs(self.tbPlayerList) do
			...
			if pPlayer then
				...
				if  nUseTime + nTime > self.MAX_DAILY_TIME or self:_CheckState() ~= 1 then
					...
					local szMsg = "Thời gian được phép hoạt động Phù Đồ Cổ Thành_Tân Thủ đã hết, ngày mai hãy đến!";
...
function tbPhuDoCoThanh_CaoThu:Init()
...
-- 增加玩家
function tbPhuDoCoThanh_CaoThu:AddPlayer(nPlayerId, nMapLevel)
...
-- 移除玩家
function tbPhuDoCoThanh_CaoThu:RemovePlayer(nPlayerId)
...
-- 开启右侧计时界面
function tbPhuDoCoThanh_CaoThu:OpenRightUI(pPlayer, nRemainFrame)
...
-- 关闭右侧计时界面
function tbPhuDoCoThanh_CaoThu:CloseRightUI(pPlayer)
...
-- 默认的回程点
function tbPhuDoCoThanh_CaoThu:GetLeaveMapPos()
...
-- 每天重置为2小时	
function tbPhuDoCoThanh_CaoThu:DailyEvent()
	...
	if (me.GetTask(self.TASK_GROUP_ID, self.TASK_USE_TIME) ~= 0) then
		Stats.Activity:AddCount(me, Stats.TASK_COUNT_PhuDoCoThanh_CaoThu, 1);
	end
...
-- 使用夜明珠回调
function tbPhuDoCoThanh_CaoThu:OnUseYemingzhu(nPlayerId)
	...
	if not self.tbPlayerList[nPlayerId] then
		Dialog:SendInfoBoardMsg(pPlayer, "<color=red>Vật phẩm chỉ có thể sử dụng bên trong Phù Đồ Cổ Thành_Tân Thủ!<color>");
		return;
	end
...
-- 需要数量
function tbPhuDoCoThanh_CaoThu:GetCostNum(pPlayer)
...
-- 加负面buff，恢复正面buff
function tbPhuDoCoThanh_CaoThu:OnMapEffect(nPlayerId, nMapLevel)
	...
	local szMsg = string.format("Bạn hiện tại đang bước vào tầng %d Phù Đồ Cổ Thành_Tân Thủ, do khí độc Phù Đồ Cổ Thành_Tân Thủ khuếch tán, bạn khó có thể di chuyển, <color=yellow>theo truyền thuyết trong giang hồ sử dụng Dạ Minh Châu có thể chống lại khí độc<color>.", nMapLevel);
	Dialog:SendBlackBoardMsg(pPlayer, "Khí độc Phù Đồ Cổ Thành_Tân Thủ khuếch tán, bạn khó có thể di chuyển, theo truyền thuyết trong giang hồ sử dụng <color=yellow>Dạ Minh Châu<color> có thể chống lại khí độc!");
	pPlayer.Msg(szMsg);
...
-- 清楚所有buff，记录正面buff属性
function tbPhuDoCoThanh_CaoThu:OnMapLeave(nPlayerId, nMapLevel)
...
function tbPhuDoCoThanh_CaoThu:_OnMapEffect(pPlayer)
...
-- 返回步骤
--[[function tbPhuDoCoThanh_CaoThu:GetBossStep()
...
-- 死亡计数
function tbPhuDoCoThanh_CaoThu:AddDeathCount()
...
function tbPhuDoCoThanh_CaoThu:ClearInfo()
...
-- 死亡事件
function tbPhuDoCoThanh_CaoThu:OnPlayerDeath()
...
-- 区域广播
function tbPhuDoCoThanh_CaoThu:Broadcast(szMsg)
...
function tbPhuDoCoThanh_CaoThu:_CheckState()
	return self._bOpen;
end

function tbPhuDoCoThanh_CaoThu:OnPlayerLogin()
...
-- 注册玩家每日事件
PlayerSchemeEvent:RegisterGlobalDailyEvent({Boss.PhuDoCoThanh_CaoThu.DailyEvent, Boss.PhuDoCoThanh_CaoThu});

-- 注册启动事件
ServerEvent:RegisterServerStartFunc(Boss.PhuDoCoThanh_CaoThu.Init, Boss.PhuDoCoThanh_CaoThu);

-- 注册死亡事件
PlayerEvent:RegisterGlobal("OnDeath", Boss.PhuDoCoThanh_CaoThu.OnPlayerDeath, Boss.PhuDoCoThanh_CaoThu);

-- 注册登陆事件
PlayerEvent:RegisterGlobal("OnLogin", Boss.PhuDoCoThanh_CaoThu.OnPlayerLogin, Boss.PhuDoCoThanh_CaoThu);
