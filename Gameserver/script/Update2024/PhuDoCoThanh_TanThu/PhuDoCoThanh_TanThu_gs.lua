Require("\\script\\Update2024\\PhuDoCoThanh_TanThu\\PhuDoCoThanh_TanThu_def.lua");

if (not MODULE_GAMESERVER) then
	return 0;
end

local tbPhuDoCoThanh_TanThu = Boss.PhuDoCoThanh_TanThu;

tbPhuDoCoThanh_TanThu.nMapId = 2109;
tbPhuDoCoThanh_TanThu.MapPos_Random =
{
{tbPhuDoCoThanh_TanThu.nMapId , 1833,3443},
{tbPhuDoCoThanh_TanThu.nMapId , 1766,3340},
{tbPhuDoCoThanh_TanThu.nMapId , 1615,3225},
{tbPhuDoCoThanh_TanThu.nMapId , 1873,3664},
{tbPhuDoCoThanh_TanThu.nMapId , 1924,3202},
};

-- 计时器触发
function tbPhuDoCoThanh_TanThu:OnTimer()
...
function tbPhuDoCoThanh_TanThu:Init()
...
-- 增加玩家
function tbPhuDoCoThanh_TanThu:AddPlayer(nPlayerId, nMapLevel)
...
-- 移除玩家
function tbPhuDoCoThanh_TanThu:RemovePlayer(nPlayerId)
...
-- 开启右侧计时界面
function tbPhuDoCoThanh_TanThu:OpenRightUI(pPlayer, nRemainFrame)
...
-- 关闭右侧计时界面
function tbPhuDoCoThanh_TanThu:CloseRightUI(pPlayer)
...
-- 默认的回程点
function tbPhuDoCoThanh_TanThu:GetLeaveMapPos()
...
-- 每天重置为2小时	
function tbPhuDoCoThanh_TanThu:DailyEvent()
	...
	if (me.GetTask(self.TASK_GROUP_ID, self.TASK_USE_TIME) ~= 0) then
		Stats.Activity:AddCount(me, Stats.TASK_COUNT_PhuDoCoThanh_TanThu, 1);
	end
...
-- 使用夜明珠回调
function tbPhuDoCoThanh_TanThu:OnUseYemingzhu(nPlayerId)
...
-- 需要数量
function tbPhuDoCoThanh_TanThu:GetCostNum(pPlayer)
...
-- 加负面buff，恢复正面buff
function tbPhuDoCoThanh_TanThu:OnMapEffect(nPlayerId, nMapLevel)
...
-- 清楚所有buff，记录正面buff属性
function tbPhuDoCoThanh_TanThu:OnMapLeave(nPlayerId, nMapLevel)
...
function tbPhuDoCoThanh_TanThu:_OnMapEffect(pPlayer)
...
-- 返回步骤
--[[function tbPhuDoCoThanh_TanThu:GetBossStep()
...
-- 死亡计数
function tbPhuDoCoThanh_TanThu:AddDeathCount()
...
function tbPhuDoCoThanh_TanThu:ClearInfo()
...
-- 死亡事件
function tbPhuDoCoThanh_TanThu:OnPlayerDeath()
...
-- 区域广播
function tbPhuDoCoThanh_TanThu:Broadcast(szMsg)
...
function tbPhuDoCoThanh_TanThu:_CheckState()
	return self._bOpen;
end

function tbPhuDoCoThanh_TanThu:OnPlayerLogin()
...
-- 注册玩家每日事件
PlayerSchemeEvent:RegisterGlobalDailyEvent({Boss.PhuDoCoThanh_TanThu.DailyEvent, Boss.PhuDoCoThanh_TanThu});

-- 注册启动事件
ServerEvent:RegisterServerStartFunc(Boss.PhuDoCoThanh_TanThu.Init, Boss.PhuDoCoThanh_TanThu);

-- 注册死亡事件
PlayerEvent:RegisterGlobal("OnDeath", Boss.PhuDoCoThanh_TanThu.OnPlayerDeath, Boss.PhuDoCoThanh_TanThu);

-- 注册登陆事件
PlayerEvent:RegisterGlobal("OnLogin", Boss.PhuDoCoThanh_TanThu.OnPlayerLogin, Boss.PhuDoCoThanh_TanThu);
