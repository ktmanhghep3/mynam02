local tbBox = Item:GetClass("Save_Box2024");


tbBox.TASKID_GROUP = 4216;
tbBox.TASKID_NIANHUA_BOX = 1;
tbBox.tbShengXiao = {"Huyền tinh cấp 1", "Huyền tinh cấp 2", "Huyền tinh cấp 3", "Huyền tinh cấp 4", "Huyền tinh cấp 5", "Huyền tinh cấp 6", "Huyền tinh cấp 7", "Huyền tinh cấp 8", "Huyền tinh cấp 9", "Huyền tinh cấp10", "Huyền tinh cấp11", "Huyền tinh cấp12"};
tbBox.tbNianHua_identify 	= {18,1,1};
	...
	for nNumber, szName in ipairs(self.tbShengXiao) do
		...
		if (nCurCount > 0) then
			...
			if nSum > nCount then
				...
				table.insert(tbOpt, {szName .. "(HT còn " .. nCurCount .. ")", self.SelectItem, self, nNumber});
			end
		end
	end
