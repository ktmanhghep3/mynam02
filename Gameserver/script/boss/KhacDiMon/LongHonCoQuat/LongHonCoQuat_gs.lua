-------------------------------------------------------
-- 文件名　：LongHonCoQuat.lua
-- 创建者　：zhangjinpin@kingsoft
-- 创建时间：2009-06-04 16:14:13
-- 文件描述：fix skill state bug 2009-6-23
-------------------------------------------------------

Require("\\script\\boss\\KhacDiMon\\LongHonCoQuat\\LongHonCoQuat_def.lua");

if (not MODULE_GAMESERVER) then
	return 0;
end

local tbLongHonCoQuat = Boss.LongHonCoQuat;

-- 计时器触发
function tbLongHonCoQuat:OnTimer()
		...
		for nPlayerId, tbPlayerMap in pairs(self.tbPlayerList) do
			...
			if pPlayer then
				...
				if  nUseTime + nTime > self.MAX_DAILY_TIME or self:_CheckState() ~= 1 then
					...
					local szMsg = "Thời gian được phép hoạt động Long Hồn Cổ Quật đã hết, ngày mai hãy đến!";
...
-- 增加玩家
function tbLongHonCoQuat:AddPlayer(nPlayerId, nMapLevel)
...
-- 移除玩家
function tbLongHonCoQuat:RemovePlayer(nPlayerId)
...
-- 开启右侧计时界面
function tbLongHonCoQuat:OpenRightUI(pPlayer, nRemainFrame)
	
	-- 右侧显示
	local szMsg = "<color=green>Thời gian hôm nay còn: <color=white>%s<color>";
...
-- 关闭右侧计时界面
function tbLongHonCoQuat:CloseRightUI(pPlayer)
...
-- 默认的回程点
function tbLongHonCoQuat:GetLeaveMapPos()
...
-- 每天重置为2小时	
function tbLongHonCoQuat:DailyEvent()
	...
	if (me.GetTask(self.TASK_GROUP_ID, self.TASK_USE_TIME) ~= 0) then
		Stats.Activity:AddCount(me, Stats.TASK_COUNT_LongHonCoQuat, 1);
	end
...
-- 使用夜明珠回调
function tbLongHonCoQuat:OnUseYemingzhu(nPlayerId)
	...
	if not self.tbPlayerList[nPlayerId] then
		Dialog:SendInfoBoardMsg(pPlayer, "<color=red>Vật phẩm chỉ có thể sử dụng bên trong Long Hồn Cổ Quật!<color>");
		return;
	end
...
-- 需要数量
function tbLongHonCoQuat:GetCostNum(pPlayer)
...
-- 加负面buff，恢复正面buff
function tbLongHonCoQuat:OnMapEffect(nPlayerId, nMapLevel)
	...
	local szMsg = string.format("Bạn hiện tại đang bước vào tầng %d Long Hồn Cổ Quật, do khí độc Long Hồn Cổ Quật khuếch tán, bạn khó có thể di chuyển, <color=yellow>theo truyền thuyết trong giang hồ sử dụng Dạ Minh Châu có thể chống lại khí độc<color>.", nMapLevel);
	Dialog:SendBlackBoardMsg(pPlayer, "Khí độc Long Hồn Cổ Quật khuếch tán, bạn khó có thể di chuyển, theo truyền thuyết trong giang hồ sử dụng <color=yellow>Dạ Minh Châu<color> có thể chống lại khí độc!");
	pPlayer.Msg(szMsg);
...
-- 清楚所有buff，记录正面buff属性
function tbLongHonCoQuat:OnMapLeave(nPlayerId, nMapLevel)
...
function tbLongHonCoQuat:_OnMapEffect(pPlayer)
...
-- 返回步骤
--[[function tbLongHonCoQuat:GetBossStep()
...
-- 死亡计数
function tbLongHonCoQuat:AddDeathCount()
...
function tbLongHonCoQuat:ClearInfo()
...
-- 死亡事件
function tbLongHonCoQuat:OnPlayerDeath()
...
-- 区域广播
function tbLongHonCoQuat:Broadcast(szMsg)
...
function tbLongHonCoQuat:_CheckState()
	return self._bOpen;
end

function tbLongHonCoQuat:OnPlayerLogin()
...
-- 注册玩家每日事件
PlayerSchemeEvent:RegisterGlobalDailyEvent({Boss.LongHonCoQuat.DailyEvent, Boss.LongHonCoQuat});

-- 注册启动事件
ServerEvent:RegisterServerStartFunc(Boss.LongHonCoQuat.Init, Boss.LongHonCoQuat);

-- 注册死亡事件
PlayerEvent:RegisterGlobal("OnDeath", Boss.LongHonCoQuat.OnPlayerDeath, Boss.LongHonCoQuat);

-- 注册登陆事件
PlayerEvent:RegisterGlobal("OnLogin", Boss.LongHonCoQuat.OnPlayerLogin, Boss.LongHonCoQuat);
