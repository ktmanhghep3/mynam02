Require("\\script\\boss\\KhacDiMon\\LongHonBiBao\\LongHonBiBao_def.lua");

if (not MODULE_GAMESERVER) then
	return 0;
end

local tbLongHonBiBao = Boss.LongHonBiBao;

-- 计时器触发
function tbLongHonBiBao:OnTimer()
		...
		for nPlayerId, tbPlayerMap in pairs(self.tbPlayerList) do
			...
			if pPlayer then
				...
				if  nUseTime + nTime > self.MAX_DAILY_TIME or self:_CheckState() ~= 1 then
					...
					local szMsg = "Thời gian được phép hoạt động Long Hồn Bí Bảo đã hết, ngày mai hãy đến!";
...
-- 增加玩家
function tbLongHonBiBao:AddPlayer(nPlayerId, nMapLevel)
...
-- 移除玩家
function tbLongHonBiBao:RemovePlayer(nPlayerId)
...
-- 开启右侧计时界面
function tbLongHonBiBao:OpenRightUI(pPlayer, nRemainFrame)
...
-- 关闭右侧计时界面
function tbLongHonBiBao:CloseRightUI(pPlayer)
...
-- 默认的回程点
function tbLongHonBiBao:GetLeaveMapPos()
...
-- 每天重置为2小时	
function tbLongHonBiBao:DailyEvent()
	...
	if (me.GetTask(self.TASK_GROUP_ID, self.TASK_USE_TIME) ~= 0) then
		Stats.Activity:AddCount(me, Stats.TASK_COUNT_LongHonBiBao, 1);
	end
...
-- 使用夜明珠回调
function tbLongHonBiBao:OnUseYemingzhu(nPlayerId)
	...
	if not self.tbPlayerList[nPlayerId] then
		Dialog:SendInfoBoardMsg(pPlayer, "<color=red>Vật phẩm chỉ có thể sử dụng bên trong Long Hồn Bí Bảo!<color>");
		return;
	end
...
-- 需要数量
function tbLongHonBiBao:GetCostNum(pPlayer)
...
-- 加负面buff，恢复正面buff
function tbLongHonBiBao:OnMapEffect(nPlayerId, nMapLevel)
	...
	local szMsg = string.format("Bạn hiện tại đang bước vào tầng %d Long Hồn Bí Bảo, do khí độc Long Hồn Bí Bảo khuếch tán, bạn khó có thể di chuyển, <color=yellow>theo truyền thuyết trong giang hồ sử dụng Dạ Minh Châu có thể chống lại khí độc<color>.", nMapLevel);
	Dialog:SendBlackBoardMsg(pPlayer, "Khí độc Long Hồn Bí Bảo khuếch tán, bạn khó có thể di chuyển, theo truyền thuyết trong giang hồ sử dụng <color=yellow>Dạ Minh Châu<color> có thể chống lại khí độc!");
	pPlayer.Msg(szMsg);
...
-- 清楚所有buff，记录正面buff属性
function tbLongHonBiBao:OnMapLeave(nPlayerId, nMapLevel)
...
function tbLongHonBiBao:_OnMapEffect(pPlayer)
...
-- 返回步骤
--[[function tbLongHonBiBao:GetBossStep()
...
-- 死亡计数
function tbLongHonBiBao:AddDeathCount()
...
function tbLongHonBiBao:ClearInfo()
...
-- 死亡事件
function tbLongHonBiBao:OnPlayerDeath()
...
-- 区域广播
function tbLongHonBiBao:Broadcast(szMsg)
...
function tbLongHonBiBao:_CheckState()
	return self._bOpen;
end

function tbLongHonBiBao:OnPlayerLogin()
...
-- 注册玩家每日事件
PlayerSchemeEvent:RegisterGlobalDailyEvent({Boss.LongHonBiBao.DailyEvent, Boss.LongHonBiBao});

-- 注册启动事件
ServerEvent:RegisterServerStartFunc(Boss.LongHonBiBao.Init, Boss.LongHonBiBao);

-- 注册死亡事件
PlayerEvent:RegisterGlobal("OnDeath", Boss.LongHonBiBao.OnPlayerDeath, Boss.LongHonBiBao);

-- 注册登陆事件
PlayerEvent:RegisterGlobal("OnLogin", Boss.LongHonBiBao.OnPlayerLogin, Boss.LongHonBiBao);
