-------------------------------------------------------
-- 文件名　：LongHonBiBaoling_1.lua
local tbMap = Map:GetClass(2107);
...
function tbMap:OnEnter(szParam)
	...
	if nUseTime >= Boss.LongHonBiBao.MAX_DAILY_TIME then
		me.NewWorld(Boss.LongHonBiBao:GetLeaveMapPos());
		me.SetFightState(0);
		return;
	end
	...
	-- 加入当前地图的列表
	Boss.LongHonBiBao:AddPlayer(me.nId, 1);
	
	-- 地图对玩家影响
	Boss.LongHonBiBao:OnMapEffect(me.nId, 1);
end;
...
function tbMap:OnLeave(szParam)
	...
	-- 清除地图效果
	Boss.LongHonBiBao:OnMapLeave(me.nId, 1);
			
	-- 移出当前地图的列表
	Boss.LongHonBiBao:RemovePlayer(me.nId);
end;
...
function tbSafeIn1:OnPlayer()
	...
	me.NewWorld(2107, 1622, 3167);--Chiến đấu
...
function tbSafeOut1:OnPlayer()
	...
	me.NewWorld(2107, 1605, 3153);--An toàn
