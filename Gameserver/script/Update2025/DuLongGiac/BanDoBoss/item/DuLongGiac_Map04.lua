local tbMap_BichBaDong = Item:GetClass("DuLongGiac_Map04");
tbMap_BichBaDong.Item_Ruong = {18,1,4041,9}; --Bản Đồ Đã Giám Định
tbMap_BichBaDong.NpcLoiVao = 8178; -- ID của NPC Lối Vào
tbMap_BichBaDong.Boss = 8179; -- ID của Boss
...
function tbMap_BichBaDong:OnUse()
    ...
    local szMsg = "Bạn muốn mở lối vào Bạch Hổ Mật Chỉ?";
    local tbOpt = {
        {"Mở lối vào Bạch Hổ Mật Chỉ", self.Active, self}, -- Tùy chọn mở lối vào
    };
...
function tbMap_BichBaDong:Active()
    ...
    local pBoss = KNpc.Add2(self.Boss, 60, -1, 2114, 1645, 3180);

    -- Tạo thông báo vị trí cụ thể với định dạng <pos=mapid,x,y>
    local szMessage = string.format("Lối vào Bạch Hổ Mật Chỉ và Boss đã được phát hiện tại <pos=%d,%d,%d>!", nMapId, nPosX, nPosY);
