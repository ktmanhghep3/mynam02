local tbItemStorage = Item:GetClass("VoCucBox");

tbItemStorage.TASKID_GROUP = 4530;
tbItemStorage.ITEM_DATA = {};

local function LoadItemData()
    local szFilePath = "\\setting\\DuLong\\ListVoCuc.txt";
...
function tbItemStorage:OnUse()
    ...
    Dialog:Say("Có thể Gửi và Rút vật phẩm thuộc Vô Cực.", tbOpt);
end
    ...
    for nId, tbItemData in pairs(self.ITEM_DATA) do
        ...
        if nCurCount > 0 then
            ...
            if nSum > nCount then
                ...
                table.insert(tbOpt, {string.format("%s (còn %d)", tbItemData.name, nCurCount), self.SelectItem, self, nId});
            end
        end
    end
