local tbItem	= Item:GetClass("CuongHoa16");
tbItem.nNumCuong = 16;
...
function tbItem:CuongHoa(tbItemObj)
	local nCount = me.GetItemCountInBags(18,1,4037,self.nNumCuong)
	if nCount < 1 then
    ...
    for i = 1, #tbItemObj do 
        ...
        Dialog:Say("Vật phẩm bỏ vào không phải Trang bị Cần Cường Hóa:\n- Vũ khí\n- Phòng cụ\n- Trang sức", {"Kết thúc đối thoại !"}); 
	    return 0;
        end 

        local szLoai = string.format("%s",CheckItem.nDetail);
	    if szLoai ~= "1" and szLoai ~= "2" and szLoai ~= "3" and szLoai ~= "7" and szLoai ~= "8" and szLoai ~= "9" and szLoai ~= "10" and szLoai ~= "4" and szLoai ~= "5" and szLoai ~= "6" and szLoai ~= "11" then
        Dialog:Say("Loại vật phẩm này chỉ dùng để cường hóa Trang bị: Vũ khí,Phòng cụ, Trang sức", {"Kết thúc đối thoại !"}); 
	    return 0;
        end 
   ...
   -- if nCount ~= 1 then 
	...
	me.ConsumeItemInBags(1, 18, 1, 4037, self.nNumCuong);
	pEquip.Bind(1);
