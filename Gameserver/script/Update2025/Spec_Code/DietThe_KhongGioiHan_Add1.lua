local tbItem = Item:GetClass("DietThe_KhongGioiHan_Add1");
...
tbItem.Level_Add = 1;
tbItem.Item_Delete = {18,1,4048,2};                       -- ID Item
...
function tbItem:Delay()
    ...
    -- Kiểm tra cấp độ hiện tại
   -- if nLevel < 1 then
      --  Dialog:SendBlackBoardMsg(me, "<color=yellow>Ngươi chưa kích hoạt Kỹ năng này, không thể dùng Nâng cấp!<color>");
       -- return 0;
    --end
    ...
    -- Nếu nhân vật vẫn còn trang bị
    if #tbWornEquip > 0 then
        local szEquipList = table.concat(tbWornEquip, "\n- "); -- Ghép danh sách trang bị
        Dialog:Say(string.format("<bclr=0,0,200><color=white>Các loại Trang bị sau phải lấy ra khỏi người mới Lĩnh ngộ được Kỹ Năng Diệt Thế Diễm Thiên Trảm:\n- %s<color>", szEquipList));
        return 0;
    end

    -- Bắt đầu quá trình lĩnh ngộ
    local tbBreakEvent = {
