...
function Item:ApplyPeelHighEquipSure()
	...
	-- SkillId : 1358
	me.AddSkillState(1358, 10, 1, self.MAX_PEEL_TIME * Env.GAME_FPS);
...
--------k tách được trang bị có hạn
local tbAbsTime = me.GetItemAbsTimeout(pEquip);
	if tbAbsTime then
	Dialog:Say("<color=yellow>Trang bị có hạn sử dụng không thể tách!<color>", {"<color=green>Ta biết rồi!<color>"});
	return -1;
	end
------------
