...
function Domain:DeclareWar_Confirm(nDomainId, nTongId)	
	...
	if  szDomainName then
		Dialog:Say("    Ngươi đã quyết định chọn <color=yellow>"..self:GetDomainName(nDomainId)..
			"<color>Là mục tiêu tuyên chiến của Bang Hội trong cuộc chiến Tranh Đoạt Lãnh Thổ lần này？\n<color=yellow>Các Bang Hội đã tuyên chiến với lãnh thổ：\n<color><color=green>"..
			szDomainName.."<color>", tbDlg);
	else
		Dialog:Say("    Ngươi chắc chắn chọn: <color=yellow>"..self:GetDomainName(nDomainId).."<color>Là mục tiêu tuyên chiến của Bang Hội trong cuộc chiến Tranh Đoạt Lãnh Thổ lần này？", tbDlg);
	end
end
