...
function FactionBattle:OnLeaveMap(nFaction)
	...
	local _,nNumOld,_ = LimitTdmp:CheckHardWare(me);
	LimitTdmp:TruNewHardWare(me,nNumOld);
	end
end
...
function FactionBattle:OnEnterMap(nFaction, bIsNewWorld)
	...
	if LimitTdmp.Tdmp_CheckNumAccount == 1 then
	local _,nNumOld,_ = LimitTdmp:CheckHardWare(me);
	LimitTdmp:WriteNewHardWare(me,nNumOld);
	end
end
...
function FactionBattle:TrapIn(pPlayer)
	...
	pPlayer.NewWorld(245, unpack(self.REV_POINT[nRandom]));
