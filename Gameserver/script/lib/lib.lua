...
function Lib:EditTabFile(szFile,nColumNameView,ValueColumView,nColumNameEdit,NewValueColumEdit)
	if not szFile then 
		return ;
	end	
	...
	local Row = Lib:SplitStr(szFileData,"\n");
	local Colum = Row[1];
	local ColumName = Lib:SplitStr(Colum,"\t");
	local szDataNew = Colum.."\n";
	if szFile then 
		local tbFile = Lib:LoadTabFile(szFile);
			for nIndex, tbRow in pairs(tbFile) do
				local NumRowData ="";
				local RowData = {};
				for i = 1,#ColumName do
					local tbTemp = {};
					tbTemp[i] = tbRow[""..ColumName[i]..""];
					table.insert(RowData, {tbTemp[i]});

				end
				
					if RowData[nColumNameView][1] == ValueColumView then
						RowData[nColumNameEdit][1] = NewValueColumEdit;
					end
				for i = 1,#ColumName do
						if i == 1 then
						NumRowData = RowData[i][1];
						elseif i == #ColumName then
						NumRowData = NumRowData.."\t"..RowData[i][1].."\n";
						else
						NumRowData = NumRowData.."\t"..RowData[i][1];
						end
				end
				szDataNew = szDataNew..NumRowData;

			end

	end	
	-- print("".. szDataNew.."");
	KFile.WriteFile(szFile, szDataNew);	
end
