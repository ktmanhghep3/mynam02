...
local CHIP_NUM		= 300;	-- 合成一張令牌需要的碎片

function tbItem_Chip:OnUse()
	local nChips		= me.GetItemCountInBags(18, 1, 185, 1);
	
	if nChips < CHIP_NUM then
		Dialog:SendInfoBoardMsg(me, "<color=red>Cần có <color><color=yellow>"..CHIP_NUM.." Mảnh lệnh bài Thiên Quỳnh Cung<color> <color=red>mới có thể hợp thành lệnh bài !<color>");
		return;
	else
