...
function tbInstancingEntrancePoint:OnDialog()
	...
	if (me.nTeamId ~= nTeamId) then
		local szMsg = "Chỉ có đội ngũ của <color=yellow>"..pOpener.szName.."<color> mới vào được mê cung dưới đất này!"
		Dialog:SendInfoBoardMsg(me, szMsg);
		return;
	end

	local tbItemId1	= {18,1,3921,1,0,0};
	local nCount1 = me.GetItemCountInBags(18,1,3921,1)
	
	if nCount1 < 1 then
		local szMsg = "Ngươi không đủ 1 <color=cyan>Lệnh Bài Khiêu Chiến Phó Bản<color>, không thể vào!"
		Dialog:SendInfoBoardMsg(me, szMsg);
	return;
	end
...
function tbInstancingEntrancePoint:Enter(nPlayerId, nNpcId, nTreasureId, nTreasureMapId, nMapTemplateId)
	...
	Task:DelItem(pPlayer, tbItemId1, 1);
