...
function tbNpc:OnDialog()
	local tbOpt = {
		--{"<color=gold>[Vận Tiêu]<color>", self.OnDialog_VanTieu, self},
		{"[Về Thương Hội]", Merchant.Npc.OnDialog_ThuongHoi, Merchant.Npc},
		{"<color=cyan>Đổi Lệnh Bài",SpecialEvent.DoiDBL.OnDialog, SpecialEvent.DoiDBL,};
		{"Kết thúc đối thoại"},
	}
	local szMsg =  [[Chủ Thương Hội: Ở đây ta có Nhiệm vụ Thương Hội:]]
	Dialog:Say(szMsg, tbOpt);
end
function tbNpc:OnDialog_VanTieu()
	local tbOpt = {
		{"Nhận thưởng",self.AddBouns,self},
		{"Kết thúc đối thoại"},
	}
	for i = 1,Huhang.MaxLevelNPC do
		table.insert(tbOpt,1 , {"".. Huhang.LevelName[i] .."",  self.OnDialog_GS_VanTieu, self,i});
	end

	local szMsg =  [[Chủ Thương Hội: Ở đây ta có Nhiệm vụ Thương Hội:]]
	Dialog:Say(szMsg, tbOpt);
end
function MerchantNpc:OnDialog_ThuongHoi()
	local tbOpt = {
...
function tbNpc:OnDialog_GS_VanTieu(nHuHangLevel)
	if Huhang.IsOpen ~= 1 then
		Dialog:Say("Chức năng này chưa mở,vui lòng thử lại sau");

	return 0;
	end
local tbNpc	= Npc:GetClass("huhang_npc");
tbNpc:OnDialog(nHuHangLevel);
end
function tbNpc:AddBouns()
	if Huhang.IsOpen ~= 1 then
		Dialog:Say("Chức năng này chưa mở,vui lòng thử lại sau");
