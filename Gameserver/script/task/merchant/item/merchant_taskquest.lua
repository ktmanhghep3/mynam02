...
function tbNpc:OnDialog()
	...
	if me.CountFreeBagCell() < 1 then
		Dialog:Say("Hành trang không đủ, hãy sắp xếp và thử lại.");
		return 0;
	end
	GeneralProcess:StartProcess("Đang thực hiện....", DELAY_TIME * Env.GAME_FPS, {self.GetQuest, self, me.nId, him.dwId}, nil, tbEvent);
end
...
function tbNpc:GetQuest(nPlayerId, nNpcId)
	...
	if nGenre > 0 and nDetail > 0 and nParticular > 0 and nLevel > 0 then
		...
		local tbFind1 = me.FindItemInBags(nGenre,nDetail,nParticular,nLevel);
		if #tbFind1 >= nNum then
			Dialog:SendBlackBoardMsg(pPlayer, string.format("Bạn đã thu thập được %s %s",nNum, szName));
		end	
	end
