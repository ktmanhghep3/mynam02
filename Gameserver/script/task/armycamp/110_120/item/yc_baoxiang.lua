...
function tbYiCengBox:OnCheckOpen(nPlayerId, nNpcId)
	...
	if (nRate < self.OPEN_NPC[1][2]) then
		...
		KTeam.Msg2Team(pPlayer.nTeamId, pPlayer.szName .." mở rương, gặp một nhóm " .. szName .."。");
	...
	elseif (nRate < self.OPEN_NPC[4][2]) then
		...
		KTeam.Msg2Team(pPlayer.nTeamId, pPlayer.szName.."Mở rương!");
	end;
...
function tbYiCengBox:OnDialog()
	...
	--开启宝箱
	GeneralProcess:StartProcess("Mở rương...", self.COST_TIME, {self.OnCheckOpen, self, me.nId, him.dwId}, {me.Msg, "Mở bị gián đoạn"}, tbEvent);
end