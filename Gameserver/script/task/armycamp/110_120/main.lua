...
function tbInstancing:OnBreath()
	...
	if (self.nTrap10Pass ~= 1) then
		for nId, nTime in pairs(self.tbFireOutTime) do
			if (nTime >= 88) then	--thay đổi 8 thành 88
	...
	if (self.nOpenZhuZiTime == 1) then
		local bAllOpen = false;	--thay đổi true thành false
		for _, nOpen in pairs(self.tbOpen) do
			if (nOpen ~= 1) then
				bAllOpen = true;	--thay đổi false thành true
				break;
			end;
		end;
		if (not bAllOpen) then
			...
			self.nOpenZhuZiTime = 0;
			Task.tbArmyCampInstancingManager:Tip2MapPlayer(self.nMapId, "Mở thất bại, phải 4 người mở cùng 1 lúc");
		else
			...
			for _, teammate in ipairs(tbPlayList) do
				...
				local szMsg = "<npc=4224>: Lại gặp lại, đúng, ta biết đường đến đó.<end><npc=4224>: Quy tắc, giúp ta giải đáp bí ẩn này, sau đó, sẽ có phần thưởng cho ngươi.<end><npc=4224>: Thật tham lam, hãy sử dụng khả năng của ngươi, để dành chiến thắng.<end>";	
...
function tbInstancing:OnClose()
	for nPlayerId, tbPlayerData in pairs(self.tbPlayerList) do
		self:KickPlayer(nPlayerId, 1, "Thời gian trong phó bản đã hết, truyền tống rời khỏi phó bản [Hải Lăng Vương Mộ]");
	end
