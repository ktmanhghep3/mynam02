...
tbInstancing.szDesc = "Chuẩn bị sẵn sàng, và dành chiến thắng";

-- 山民 说的话
tbInstancing.tbShanMinText = {
	{"Phải vượt qua ải mới có thể đi tới nơi đó!", ""},
	{"Nơi này không thể đi tới!", "Đi đến Bích Ngô Phong!"},
	{"Nơi này không thể đi tới!", "Đi đến Chu Thần Phong"},
	{"Nơi này không thể đi tới!", "Đi đến Linh Hạt Phong!"},
	{"Nơi này không thể đi tới!", "Đi đến Thiên Tuyệt Phong!"},
	{"Bạn có thể đến nơi này, rất tốt!"},
	{"Đã tới!"},
}
...
function tbInstancing:OnBreath()
	...
	if (self.nCurSec % 600 == 0) then
		Task.tbArmyCampInstancingManager:Tip2MapPlayer(self.nMapId, "Phó bản "..self.tbSetting.szName.." sẽ đóng trong"..math.floor((self.tbSetting.nInstancingExistTime-self.nCurSec)/60).." phút nữa");
	end
	-- 指引的人每10秒说一次话
	if ((self.nCurSec - 1) % 5 == 0) then
		self:NpcTimerSay(self.nTaoHuaLinZhiYin, "Phía trước rất nguy hiểm, hãy đi theo ta!");
		self:NpcTimerSay(self.nBiWuFengZhiYin, "Đại hiệp xin dừng bước!");
		self:NpcTimerSay(self.nShenZhuFengZhiYin, "Khoan đi trước, phía trước có mai phục!");
		self:NpcTimerSay(self.nLingXieFengZhiYin, "Hóa giải các bức tượng!");
		self:NpcTimerSay(self.nTianJueGongZhiYin, "Tới đây thỉnh giáo!");
	end;
...
function tbInstancing:OnClose()
	for nPlayerId, tbPlayerData in pairs(self.tbPlayerList) do
		self:KickPlayer(nPlayerId, 1, "Thời gian trong phó bản đã hết, truyền tống rời khỏi phó bản [Bách Man Sơn]");
	end
...
function tbInstancing:OnPlayerEnter(nPlayerId)
	...
	Task.tbArmyCampInstancingManager:ShowTip(pPlayer, "Vào phó bản "..self.szOpenerName.." tại "..self.szRegisterMapName.." mở bởi "..self.tbSetting.szName .. "", 20);
	
	-- 计时面板
	if (not self.nCurSec) then -- 在报名的一秒钟以内进入副本，self.nCurSec还没经过OnBreath生成，为nil 则在此处生成
		self.nCurSec = 0;
	end;
	Dialog:SetTimerPanel(pPlayer, "<color=Gold>PHÓ BẢN QUÂN DOANH<color>\n<color=White>Đóng phó bản trong: <color>", (self.tbSetting.nInstancingExistTime-self.nCurSec));
