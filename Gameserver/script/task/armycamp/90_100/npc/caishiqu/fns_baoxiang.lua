...
tbFnsTreasureBox.COST_TIME			= 0.5	--开箱需要的时间
tbFnsTreasureBox.TIRED_DURATION		= 0.5 * Env.GAME_FPS;	-- 劳累持续时间
...
function tbFnsTreasureBox:DecreaseLockLayer(pPlayer, pNpc)
	...
	KTeam.Msg2Team(pPlayer.nTeamId, pPlayer.szName.." mở lớp khóa thứ <color=yellow>" .. tbNpcData.CUR_LOCK_COUNT .. "<color> bảo rương!");
	...
	--最后一层
	if(tbNpcData.CUR_LOCK_COUNT == 5) then 
		KTeam.Msg2Team(pPlayer.nTeamId, "<color=yellow>Bảo rương đã được mở<color>!");
...
function tbFnsTreasureBox:OnDialog()
	...
	--不存在
	if not pOpener then
		local szMsg = "Không thể mở rương của người khác!"
	...
	if (me.nTeamId == 0) then
		local szMsg = "Tổ đội mới mở được bảo rương!"
		Dialog:SendInfoBoardMsg(me, szMsg);
		return;
	end
	--宝箱是否是所在队伍的宝箱
	if (me.nTeamId ~= nTeamId) then
		local szMsg = "Chỉ có tổ đội của <color=yellow>"..pOpener.szName.."<color> mới có thể mở bảo rương!"
		Dialog:SendInfoBoardMsg(me, szMsg);
		return;
	end
	--间隔时间是否到	
	...
	if (nRet ~=  -1) then
		Dialog:SendInfoBoardMsg(me, "<color=red>Bạn quá mệt mỏi cần nghỉ ngơi 1 lúc mới có thể tiêp tục mở!<color>");
		return;
	end
	--打断开启事件
	local tbEvent = 
	...
	--开启宝箱
	GeneralProcess:StartProcess("Mở bảo rương", tbFnsTreasureBox.COST_TIME, {self.OnCheckOpen, self, me.nId, him.dwId}, {me.Msg, "Mở bị gián đoạn"}, tbEvent);
end