...
function tbNpc:OnDialog()
	...
	if (tbInstancing.tbBarrierPairs[nTrapIndex][3] == 1) then
		Task.tbArmyCampInstancingManager:ShowTip(me, "Cơ quan đã được mở!");
		return;
	end
	...
	GeneralProcess:StartProcess("Đang mở cơ quan", 0.1 * 18, {self.OnOpen, self, me.nId, nSubWorld, nTrapIndex}, {me.Msg, "Mở bị gián đoạn!"}, tbEvent);		
end;
...
function tbNpc:OnOpen(nPlayerId, nMapId, nTrapIndex)
	...
	Task.tbArmyCampInstancingManager:ShowTip(pPlayer, "Sau 1 tiếng nổ lớn, cổng đã được mở!");
end;
