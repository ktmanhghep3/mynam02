...
function JbExchange:AddOneBill(nPlayerId, nPrice, nCount, nType)
	...
	if (0 == bRet) then
		local szTilte		= "Thông báo giao dịch đồng thất bại";
		local szTypeBack	= "";
		local szType		= "";
		
		if (nType == self.SELLTYPE) then
			szTypeBack = "nhận lại đồng";
			szType	   = "Bán đồng"; 
		elseif (nType == self.BUYTYPE) then
			szTypeBack = "nhận lại bạc";
			szType	   = "Mua đồng";
		end
		local szContent = "Xin chào, giao dịch đồng của bạn đã thất bại, " .. szTypeBack .. ", xin lỗi vì sự bất tiện này:";		
		szContent	= szContent .. "\nGiao dịch: " .. szType .. "\nSố lượng: " .. nCount .. "\nGiá: " .. nPrice .. "\nThời gian: " .. GetLocalDate("%Y%m%d");
