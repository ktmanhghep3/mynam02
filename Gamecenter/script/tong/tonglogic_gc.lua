...
function Tong:DisbandTong_GC(nTongId)
	local cTong = KTong.GetTong(nTongId)
	if not cTong then
		return 0
	end
	...
	local cMasterKin = KKin.GetKin(nMasterKin);
	local cMember 	= cMasterKin.GetMember(cMasterKin.GetCaptain());
	local nMasterId = cMember.GetPlayerId();
...
-- 用来给帮会和帮会下属家族随机周任务
function Tong:PerTongWeeklyAction_GC(cTong, nTongId)
	local nLowTask, nMidTask, nHighTask = self:RandWeeklyAction(cTong);	-- 为帮会随机低中高三个档次的目标
	...
	GlobalExcute{"Tong:PerTongWeeklyAction_GS", nTongId, nLowTask, nMidTask, nHighTask};
end
...
-- 随机可以执行的任务
function Tong:RandWeeklyAction(cTong)
	local nLowTask = cTong.GetLowWeeklyTask();
	local nMidTask = cTong.GetMidWeeklyTask();
	local nHighTask = cTong.GetHighWeeklyTask();
	local nTask = MathRandom(#self.tbWeeklyAction[1]);
	while (not nTask or nTask == 0 or nTask == nLowTask) do
		nTask = MathRandom(#self.tbWeeklyAction[1]);
	end
	nLowTask = nTask;
	nTask = MathRandom(#self.tbWeeklyAction[2]);
	while (not nTask or nTask == 0 or nTask == nMidTask) do
		nTask = MathRandom(#self.tbWeeklyAction[2]);
	end
	nMidTask = nTask;
	nTask = MathRandom(#self.tbWeeklyAction[3]);
	while (not nTask or nTask == 0 or nTask == nHighTask ) do
		nTask = MathRandom(#self.tbWeeklyAction[3]);
	end
	nHighTask = nTask;
	return nLowTask, nMidTask, nHighTask;
end
...
function Tong:CheckAndSpiltStock(nTongId)
	local pTong = KTong.GetTong(nTongId);
	if not pTong then
		return 0;
	end
	...
	if nBuildFund / nTongStock > self.SPILT_STOCK_MIN_PRICE then
...
function Tong:_AddTongBuildFund(nKinId, nMemberId, nFund, nTranceRate)
	...
	local nPersonalStock = pMember.GetPersonalStock();
	if not pTong then
		pMember.SetPersonalStock(nPersonalStock + nFund);	-- 没有帮会的时候股价恒定为1，也就是等于资产，全部转化为成员资产
...
function Tong:SyncTotalStock(nTongId)
	...
	if nTotalStock ~= pTong.GetTotalStock() then
		...
		pTong.SetTotalStock(nTotalStock);
		GlobalExcute{"Tong:SyncTotalStock_GS2", nTongId, nTotalStock, tbRet};
	end
end
...
function Tong:AddGreatBonus_GC(nTongId, nMoney, nPlayerId)
	...
	if nBonus > 10000 then
		if nPlayerId then
			...
			Lib:WriteLog("Tong", "增加帮会奖励基金"..nBonus, "帮会"..pTong.GetName());
		end
	end
