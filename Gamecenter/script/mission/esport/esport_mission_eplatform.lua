EPlatForm.SnowFight = EPlatForm.SnowFight or {};

function EPlatForm.SnowFight:GetNewMission()
	return Lib:NewClass(EPlatForm.SnowFight);	
end

-- nMatchType 表示活动类型是混战还是组队pvp
-- 若是混战那么就是2
-- 若是组队pvp就是3,4
function EPlatForm.SnowFight:OpenMission(tbEnterPos, tbLeavePos, nMatchType, nReadyId)
	self.tbMission = self.tbMission or Lib:NewClass(Esport.Mission);
	self.tbMission:__open(tbEnterPos, tbLeavePos, nMatchType);
...
function EPlatForm.SnowFight:IsOpen()
...
function EPlatForm.SnowFight:StartGame()
	self.tbMission:__start();
end

function EPlatForm.SnowFight:OnMissionClose()
...
function EPlatForm.SnowFight:GetEnterPos()
	return self.tbEnterPos;
end

function EPlatForm.SnowFight:GetGroupCount()
	return self.nGroupCount or 0;
end

function EPlatForm.SnowFight:JoinGame(tbGroup, nCampId)
...
function EPlatForm.SnowFight:CloseGame()
	self.tbMission:Close();
end

function EPlatForm.SnowFight:GetPlayerGroupId(pPlayer)
	if (not pPlayer) then
		return -1;
	end
...
function EPlatForm.SnowFight:KickPlayer(pPlayer)
	if (not pPlayer) then
		return -1;
	end
