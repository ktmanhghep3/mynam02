...
function DragonBoat:OnPlayerBaseType3(nType, nGroup, nPoint)
	...
	tbMis:SkillItemClose(nGroup, nPoint);
	Dialog:SendBlackBoardMsg(me, string.format("你从河道中意外的打捞到%s！", KItem.GetNameById(tbItem[1], tbItem[2], tbItem[3], tbItem[4])));
	me.Msg(string.format("<color=blue>你从河道中意外的打捞到%s！<color>", KItem.GetNameById(tbItem[1], tbItem[2], tbItem[3], tbItem[4])));		
end
