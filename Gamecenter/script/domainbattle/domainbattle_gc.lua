...
function Domain:SortMemberScore_GC()
	...
	for nTongId, tbPlayerSet in pairs(self.tbPlayer) do	
		...
		if pTong then
			...
			local nPercent = 0;
			for i, tbInfo in pairs(self.DOMAINBATTLE_SCORE_RATE_TABLE) do
...
function Domain:AddDomainNews()
	...
	local nSort = 1;
	local nBefore = 0;
	szMsg = os.date("%Y-%m-%d- <color=yellow>Chiến báo Lãnh thổ tranh đoạt chiến<color>\n", GetTime());
	while (nCount <= 10 or nSort <= 10) and (#tbResult >= nCount) do
		szMsg = szMsg.."\n"..Lib:StrFillL("Hạng "..nSort..": ", 8).."<color=yellow>"..Lib:StrFillL(tbResult[nCount].szName, 14).."<color> chiếm đóng <color=green>"..tbResult[nCount].nKey.."<color> lãnh thổ"
	...
	if #tbVillageResult > 0 then
		szMsg = szMsg.."\n\n<color=yellow>Tân Thủ Thôn chiến báo<color>\n\n";
		for _, tbInfo in ipairs(tbVillageResult) do
			szMsg = szMsg..string.format("<color=yellow>%s<color> chiếm lĩnh <color=green>%s<color>, nhận được quyền tiến đánh lãnh thổ\n", 
				tbInfo.szTongName, tbInfo.szVillage);
		end
	end
	
	Task.tbHelp:AddDNews(Task.tbHelp.NEWSKEYID.NEWS_STARTDOMAIN, "Chiến báo Lãnh thổ tranh đoạt chiến", szMsg, nEndTime, nAddTime);
end
	...
	if nOrgOwnerId ~= 0 then
		...
		if pTong then
			...
			if szName then
				...
				_G.TongLog(pTong.GetName(), Log.emKTONG_LOG_TONGSTRUCTURE, string.format("Bang hội %s bị mất lãnh thổ [%s]", pTong.GetName(), szName));
			end
		...
		if pUnion then
			...
			local szName = tbDomainName[nDomainId];
			if szName then
				_G.TongLog(pUnion.GetName(), Log.emKTONG_LOG_TONGSTRUCTURE, string.format("Liên minh %s bị mất lãnh thổ [%s]", pUnion.GetName(), szName));
			end
		end
	end
	local nTime = GetTime();
	...
	if (nNewOwnerId ~= 0) then
		...
		if pTong then
			...
			if szName then
				...
				_G.TongLog(pTong.GetName(), Log.emKTONG_LOG_TONGSTRUCTURE, string.format("Bang hội %s chiếm lĩnh lãnh thổ [%s]", pTong.GetName(), szName));
			end
		...
		if pUnion then
			...
			local szName = tbDomainName[nDomainId]
			if szName then
				_G.TongLog(pUnion.GetName(), Log.emKTONG_LOG_TONGSTRUCTURE, string.format("Liên minh %s chiếm lĩnh lãnh thổ [%s]", pUnion.GetName(), szName));
			end
		end
	end
...
function Domain:SetTongAward_GC(nTongId, nSelfKinId, nSelfMemberId, nLevel, nPlayerId)
	...
	if (cTong and nLevel > 0 and nWastage > 0 and nTongBuildFund >= 0 and 
		cTong.GetDomainAttendNo() == nGblBattleNo and nDomainAwardAmount > 0) then 
			print("Bang hội : "..cTong.GetName().." bổ sung quân hưởng đến: "..nDomainAwardAmount);
...
function Domain:GetTongAward_GC(szReason)
	...
	table.sort(tbTongAmount, function(a, b) return (a.nAmount > b.nAmount) end);
	if (not tbTongAmount[1] or tbTongAmount[1].nAmount == 0) then
		return;
	end
	for i = 1, 10 do
		...
		Dbg:Output("Domain", "tbChaoTingYuShi", string.format("Nhiệm vụ Bá chủ ấn bang hội %s cấp cho quỹ xây %s", cTong.GetName(), self.BAZHU_AWARD[i]));
	end
