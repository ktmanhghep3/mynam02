...
function EventKind:ExeStartFun()
	...
	for _, varTask in pairs(tbTask) do
		...
		SpecialEvent.ExtendAward:RegExecute(szType, {EventKind.ExeTaskFinish, self}, nNeedFree, {EventKind.ExeTaskCheckFreeCount, self})
	end
...
function EventKind:ExeEndFun()
	...
	for _, varTask in pairs(tbTask) do
		...
		local szType = tbTask[1];
		local nNeedFree = tonumber(tbTask[2]) or 0;
		SpecialEvent.ExtendAward:UnRegExecute(szType, {EventKind.ExeTaskFinish, self}, nNeedFree, {EventKind.ExeTaskCheckFreeCount, self})
	end
